...
int main() {
    ...
    for (int i = 0; i < m; i++) {
        ...
        if (opt == Operation::Insert) {
            ...
            ans = get_rank(get_prev(tree.root, x)).right + 1;
        } else if (opt == Operation::GetNumberByRank) {
            ans = get_number_by_rank(tree.root, x);
        } else if (opt == Operation::Previous) {
            ans = get_prev(tree.root, x)->value;
        } else if (opt == Operation::Next) {
            ans = get_next(tree.root, x)->value;
        }
        ans_xorsum = ans_xorsum ^ ans;
    }
    printf("%d\n", ans_xorsum);
    return 0;
}